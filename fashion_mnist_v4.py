...
def build_srwn_configs(dropout_rate: float):
    # v4 keeps topology fixed and pushes width with dropout regularization.
    return [
        {
            "name": "wide_d01_r3_h64_w3",
            "family": "wide_dropout",
            "rows": 3,
            "cols": 4,
            "hidden_dim": 64,
...
def plot_accuracy_bar(metrics, out_dir):
    ...
    plt.title("Fashion-MNIST v4: Wide SRWN + Dropout vs CNN (accuracy)")
    plt.axhline(y=metrics["cnn"]["test"]["test_accuracy"], color="#d62728", linestyle="--", alpha=0.7)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fashion_v4_accuracy_bar.png"), dpi=140)
    plt.close()
...
def plot_compute_bar(metrics, out_dir):
    ...
    plt.title("Fashion-MNIST v4: Wide SRWN + Dropout vs CNN (compute)")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fashion_v4_compute_bar.png"), dpi=140)
    plt.close()
...
def plot_acc_vs_macs(metrics, out_dir):
    ...
    plt.ylabel("Test accuracy")
    plt.title("Fashion-MNIST v4: Accuracy vs compute")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fashion_v4_acc_vs_macs.png"), dpi=140)
    plt.close()
...
def make_verdict(metrics):
    ...
    return {
        ...
        "beats_cnn_on_accuracy": beats_acc,
...
def render_markdown(metrics, verdict):
    ...
    for entry in metrics["srwn"]:
        ...
        test = entry["test"]
        comp = entry["compute"]
        beats = "yes" if test["rightmost_test_accuracy"] >= cnn_acc else "no"
        lines.append(
            f"| {entry['name']} | {cfg['hidden_dim']} | {cfg.get('dropout_rate', 0.0):.2f} | "
            f"{test['rightmost_test_accuracy']:.4f} | {test['halted_test_accuracy']:.4f} | {test['mean_halt_wave']:.4f} | "
            f"{comp['adaptive_estimated_macs']:.2f} | {entry['params']} | {beats} |"
        )
    ...
    if verdict["beats_cnn_on_accuracy"]:
        lines.append(
            f"At least one SRWN variant matched or exceeded CNN accuracy ({cnn_acc:.4f})."
        )
    else:
        ...
        lines.append(f"Best SRWN: {best['name']} at {best['test']['rightmost_test_accuracy']:.4f} (gap {gap:.4f}).")
...
def run(config):
    ...
    srwn_metrics = []
    for variant in build_srwn_configs(config["dropout_rate"]):
    ...
    metrics_path = os.path.join(config["out_dir"], "fashion_v4_metrics.json")
    verdict_path = os.path.join(config["out_dir"], "fashion_v4_verdict.json")
    md_path = os.path.join(config["out_dir"], "fashion_v4_results.md")
    ...
    return {
        ...
        "markdown": md_path,
        "plots": {
            "accuracy_bar": os.path.join(config["out_dir"], "fashion_v4_accuracy_bar.png"),
            "compute_bar": os.path.join(config["out_dir"], "fashion_v4_compute_bar.png"),
            "acc_vs_macs": os.path.join(config["out_dir"], "fashion_v4_acc_vs_macs.png"),
        },
        "headline": {
            "cnn": verdict["cnn"],
            "best_srwn_accuracy": {
...
def main():
    parser = argparse.ArgumentParser(description="Fashion-MNIST v4: wide SRWN + dropout")
